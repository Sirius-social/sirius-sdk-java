...
public abstract class LazySodium implements
    ...
    @Override
    public boolean cryptoBox(byte[] cipherText, byte[] message, long messageLen, byte[] nonce, byte[] publicKey, byte[] secretKey) {
        if (messageLen < 0 || messageLen > message.length) {
            throw new IllegalArgumentException("messageLen out of bounds: " + messageLen);
        }
    ...
    @Override
    public boolean cryptoBoxOpen(byte[] message, byte[] cipherText, long cipherTextLen, byte[] nonce, byte[] publicKey, byte[] secretKey) {
        if (cipherTextLen < 0 || cipherTextLen > cipherText.length) {
            throw new IllegalArgumentException("cipherTextLen out of bounds: " + cipherTextLen);
        }
        return successful(getSodium().crypto_box_open(message, cipherText, cipherTextLen, nonce, publicKey, secretKey));
    }
