...
public class RaftConsensusServiceImpl implements RaftConsensusService {
    ...
    public VoteResponse preVote(VoteRequest request) {
        LOG.info("RaftConsensusServiceImpl preVote");
        raftNode.getLock().lock();
        try {
            VoteResponse responseBuilder = new VoteResponse();
            responseBuilder.setGranted(false);
            responseBuilder.setTerm(raftNode.getCurrentTerm());
            boolean containsServer  = ConfigurationUtils.containsServer(raftNode.getConfiguration(), request.getServerId());
            if (!containsServer) {
    ...
    public VoteResponse requestVote(VoteRequest request) {
        LOG.info("RaftConsensusServiceImpl requestVote");
        raftNode.getLock().lock();
        try {
            VoteResponse responseBuilder = new VoteResponse();
            responseBuilder.setGranted(false);
            ...
            boolean logIsOk = request.getLastLogTerm() > raftNode.getLastLogTerm()
                    || (request.getLastLogTerm() == raftNode.getLastLogTerm()
                    && request.getLastLogIndex() >= raftNode.getRaftLog().getLastLogIndex());
            if (raftNode.getVotedFor() == "" && logIsOk) {
                raftNode.stepDown(request.getTerm());
    ...
    public AppendEntriesResponse appendEntries(AppendEntriesRequest request) {
        LOG.info("RaftConsensusServiceImpl appendEntries");
        raftNode.getLock().lock();
        try {
            AppendEntriesResponse responseBuilder
            ...
            raftNode.stepDown(request.getTerm());
            if (raftNode.getLeaderId().equals("")) {
                raftNode.setLeaderId(request.getServerId());
                LOG.info("new leaderId={}, conf={}",
                        raftNode.getLeaderId(),
                        raftNode.getConfiguration().toString());
            }
            if (!raftNode.getLeaderId().equals(request.getServerId())) {
...
//                    null, raftNode.getRaftLog().getFirstLogIndex());
    ...
    public InstallSnapshotResponse installSnapshot(InstallSnapshotRequest request) {
        LOG.info("RaftConsensusServiceImpl installSnapshot");
        InstallSnapshotResponse responseBuilder
        ...
        try {
            ...
            randomAccessFile.write(request.getData());
    ...
    private void advanceCommitIndex(AppendEntriesRequest request) {
        LOG.info("RaftConsensusServiceImpl advanceCommitIndex");
            ...
            for (long index = raftNode.getLastAppliedIndex() + 1;
                ...
                if (entry != null) {
                    if (entry.getType() == EntryType.ENTRY_TYPE_DATA) {
                        raftNode.getStateMachine().apply(entry.getData());
