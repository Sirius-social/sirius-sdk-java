...
import com.github.wenweihu86.raft.models.LogEntry;
...
public class SegmentedLog {
    ...
    public SegmentedLog(String raftDataDir, int maxSegmentFileSize) {
        ...
        if (metaData == null) {
            ...
            metaData = new LogMetaData().setFirstLogIndex(1L);
        }
    }
    ...
    public long append(List<LogEntry> entries) {
        ...
        for (LogEntry entry : entries) {
            newLastLogIndex++;
           //TODO int entrySize = entry.getSerializedSize();
            int entrySize = entry.getData().length;
     ...
     * @param commitIndex
     */
    public void updateMetaData(Long currentTerm, String votedFor, Long firstLogIndex, Long commitIndex) {
        LogMetaData builder = new LogMetaData(this.metaData);
        if (currentTerm != null) {
