 ...
 */
public abstract class AbstractCloudCoProtocolTransport extends AbstractCoProtocolTransport {

    public static final String THREAD_DECORATOR = "~thread";
    public static final String PLEASE_ACK_DECORATOR = "~please_ack";

    public AbstractCloudCoProtocolTransport(AgentRPC rpc) {
        this.rpc = rpc;
    }
    ...
    public List<Pair<Boolean, String>> sendMany(Message message, List<Pairwise> to) throws SiriusPendingOperation {
        ...
        for (Pairwise p : to) {
            batches.add(new RoutingBatch(Collections.singletonList(p.getTheir().getVerkey()), p.getTheir().getEndpointAddress(), p.getMe().getVerkey(), p.getTheir().getRoutingKeys()));
        }
        if (!isSetup) {
            throw new SiriusPendingOperation("You must Setup protocol instance at first");
        }
